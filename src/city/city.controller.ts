...
import {  getAllcity, getOnecity ,deletecity,insertcity,updatecity,getAllcityrelation} from "./city.service";
...
export const getcityrelation = async(c: Context) => {
    try {
        const data = await getAllcityrelation()
        return c.json(data, 200)
    } catch (err) {
        return c.json({ "message": err },500)
    }
